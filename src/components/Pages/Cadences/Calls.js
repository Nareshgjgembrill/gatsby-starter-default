...
const Calls = ({
  ...
  if (searchInput) {
    prospectsFilter = `&filter[user][id]=${userIds}&filter[touchType]=call&filter[q]=${encodeURIComponent(
      searchInput
    )}`;
  } else {
    prospectsFilter = `&filter[user][id]=${userIds}&filter[touchType]=call`;
  }
  ...
  const handleRowToolbarButtonAction = (action, prospect) => {
    if (
      ...
      if (pauseIds.length === 0) {
        ...
        setPopupMessage('Prospects are not assigned to any Cadence.');
        return;
      }
    }
  ...
  const ProspectsDatacolumns = React.useMemo(
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [location.pathname, location.search, prospectSearchUrl, filters]
  );
