...
const Emails = ({
  ...
  if (searchInput) {
    prospectsFilter = `&filter[user][id]=${userIds}&filter[touchType]=email&filter[q]=${encodeURIComponent(
      searchInput
    )}`;
  } else {
    prospectsFilter = `&filter[user][id]=${userIds}&filter[touchType]=email`;
  }
  ...
  ] = useLazyQuery(EMAIL_PREVIEW_PROSPECT, {
    notifyOnNetworkStatusChange: true,
    fetchPolicy: 'cache-first',
  });
  ...
  const previewEmailData = useMemo(
    () =>
      previewData && previewData?.preview?.data[0]
        ? previewData.preview.data[0]
        : [],
    [previewData]
  );
  const handleEmailPreview = (id, touchId) => {
    let previewFilter;
    if (touchId !== null && touchId > 1) {
      previewFilter = `filter[user][id]=${userIds}&filter[memberId]=${id}&filter[cadence][id]=${cadenceId}&filter[touchStepNo]=<=2&sort[id]=desc&page[limit]=1`;
    } else {
      previewFilter = `filter[user][id]=${userIds}&filter[memberId]=${id}&filter[cadence][id]=${cadenceId}&sort[id]=desc&page[limit]=1`;
    }
    emailPreview({
      variables: {
  ...
  const handleMoveProspectRequestCallback = (
    ...
    if (requestSuccess) {
      ...
      if (
        movedCountList?.length === response?.assignOrMoveProspect?.data?.length
      ) {
        notify(
  ...
  const ProspectsDatacolumns = React.useMemo(
    () => [
      ...
      {
        Header: ['total', 'active', 'paused'].includes(filters)
          ? 'Last Email Outcome'
          : 'Email Outcome',
        accessor: 'outCome',
        width: '14%',
        Cell: function (props) {
          return (
            <span className="text-break">
              {props.row.original.outcome || 'N/A'}
              <i
                className="fas fa-envelope fa-sm text-email pointer ml-2"
                title="Prospect Email Details"
                onClick={() => {
                  handleEmailPreview(
                    props.row.original.id,
                    props.row.original.currentTouchId
                  );
                }}
                hidden={!props.row.original.outcome}
              ></i>
            </span>
          );
        },
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [location.pathname, prospectSearchUrl, filters]
  );
  ...
  return (
    <div>
      ...
      <TemplatePreview
        ...
        previewData={previewEmailData}
        previewLoading={previewLoading}
        previewError={previewError}
        header="Prospect Email Details"
        type="view"
      />
    </div>
  );
