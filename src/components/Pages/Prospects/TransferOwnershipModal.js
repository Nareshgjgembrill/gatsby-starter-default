...
import { FETCH_ALL_TEAM_USER_QUERY } from '../../queries/ProspectsQuery';
...
const OptionComponent = ({ selectedUserId, users, currentUserId, toUser }) => {
  ...
  if (selectedUserId) {
    ...
    userList =
      ...
      users.data &&
      users.data.filter(
        (user) => user.id !== currentUserId && user.id !== parseInt(toUser)
      );
  }
  return (
    <>
...
const TransferOwnershipModal = ({
  ...
  const [fetchUserList, { data: userData }] = useLazyQuery(
    FETCH_ALL_TEAM_USER_QUERY,
    {}
  );
  ...
  useEffect(() => {
    ...
    if (
      showModal &&
      user &&
      (user.isManagerUser === 'N' || user.isAdminUser === 'N')
    ) {
      fetchUserList({ variables: { currentUserId, limit: 200 } });
    }
        ...
        <Form name="transferownership" innerRef={formRef}>
          {selectedUserName && (
            <p className="mb-2 text-center">
              <span className="bg-color-yellow">
                Transfer prospect(s) on behalf of <b>{selectedUserName}</b>
              </span>
            </p>
          )}
          <FormGroup row>
          ...
          <FormGroup row>
            ...
            <Col sm={12} lg={9}>
              ...
              >
                <OptionComponent
                  users={userData?.users ? userData.users : users}
                  currentUserId={currentUserId}
                  toUser={fromUser}
                ></OptionComponent>
              </Input>
